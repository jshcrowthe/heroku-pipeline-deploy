...
main() {
  ...
  init_ssh;
  init_git "$WERCKER_HEROKU_PIPELINE_DEPLOY_USER" "$WERCKER_HEROKU_PIPELINE_DEPLOY_USER";
  ...
  if [ -n "$WERCKER_HEROKU_PIPELINE_DEPLOY_KEY_NAME" ]; then
    use_wercker_ssh_key "$ssh_key_path" "$WERCKER_HEROKU_PIPELINE_DEPLOY_KEY_NAME";
  else
  ...
  if [ -z "$WERCKER_HEROKU_PIPELINE_DEPLOY_KEY_NAME" ]; then
...
init_wercker_environment_variables() {
  ...
  if [ -z "$WERCKER_HEROKU_PIPELINE_DEPLOY_USER" ]; then
    if [ ! -z "$HEROKU_USER" ]; then
      export WERCKER_HEROKU_PIPELINE_DEPLOY_USER="$HEROKU_USER";
    else
      export WERCKER_HEROKU_PIPELINE_DEPLOY_USER="heroku-deploy@wercker.com";
    fi
  fi

  if [ -z "$WERCKER_HEROKU_PIPELINE_DEPLOY_SOURCE_DIR" ]; then
    export WERCKER_HEROKU_PIPELINE_DEPLOY_SOURCE_DIR="$WERCKER_ROOT";
    debug "option source_dir not set. Will deploy directory $WERCKER_HEROKU_PIPELINE_DEPLOY_SOURCE_DIR";
  else
    warn "Use of source_dir is deprecated. Please make sure that you fix your Heroku deploy version on a major version."
    debug "option source_dir found. Will deploy directory $WERCKER_HEROKU_PIPELINE_DEPLOY_SOURCE_DIR";
  fi
}
